...
class SelfDrivingDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # # Padding to ensure a fixed number of boxes
        # max_boxes = 20  # Adjust as needed
        # num_boxes = boxes.shape[0]
        # if num_boxes < max_boxes:
        #     # Pad with dummy boxes and labels
        #     pad_boxes = torch.zeros((max_boxes - num_boxes, 4), dtype=torch.float32)
        #     pad_labels = torch.zeros((max_boxes - num_boxes,), dtype=torch.int64)
...
def get_data_loaders(
    ...
    data_transforms = {
        "train": get_transform(SelfDrivingDataset.mean, SelfDrivingDataset.std, train=True),
        "valid": get_transform(SelfDrivingDataset.mean, SelfDrivingDataset.std, train=False),
        "test": get_transform(SelfDrivingDataset.mean, SelfDrivingDataset.std, train=False),
    }

    # Create train and validation datasets
    train_data = SelfDrivingDataset(
        folder, 
